namespace Travel.Core.Controllers
{
	...
	public class AirportController : IAirportController
	{
		...
		{
            int startingBags = bagIndices.Count();
			var passenger = this.airport.GetPassenger(username);
			var trip = airport.Trips.FirstOrDefault(t=>t.Id == tripId);

			var checkedIn = airport.Trips.Any(a=>a.Airplane.Passengers.Any(p => p.Username == username));
			...
			return
				$"Checked in {passenger.Username} with {startingBags - confiscatedBags}/{startingBags} checked in bags";
		}
